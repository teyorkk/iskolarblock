...
interface UserSidebarDesktopProps {
...
export function UserSidebarDesktop({
  ...
  pathname,
  onLogout,
}: UserSidebarDesktopProps): React.JSX.Element {
  ...
  return (
    <>
      ...
      >
        <UserSidebarHeader isCollapsed={isCollapsed} />
        ...
        <div className="p-4 overflow-y-auto h-[calc(100vh-88px)]">
          <UserSidebarProfile user={user} isCollapsed={isCollapsed} />
      ...
      >
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-gray-900">
            {userNavigation.find((item) => item.href === pathname)?.name ||
              "Dashboard"}
          </h1>
        </div>
      </motion.header>
